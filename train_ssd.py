...
def parse_args():
    ...
    parser.add_argument('--lr-decay-epoch', type=str, default='160,200',
                        help='epochs at which learning rate decays. default is 60,100.')
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        ...
        val_dataset = VOCLike(root='./VOCtemplate', splits=[(2018, 'trainval')])
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        while lr_steps and epoch >= lr_steps[0]:
            ...
            logger.info("[Epoch {}] Set l earning rate to {}".format(epoch, new_lr))
        ce_metric.reset()
